 ...
 * size. Every truncation therefore has to happen on bytes, not
 * characters, or long Persian names silently overflow the limit.
 */
function cutToBytes(value, budget) {
  ...
  // Do not split a multibyte character in half.
  ...
  const text = decoder.decode(bytes.slice(0, keep));
 ...
 */
function navRow(back = "home", help = "home") {
  return [nav(back, help)];
}

/* Split a flat list of buttons into rows of at most `size`. */
...
<<<<<<< HEAD
     ...
     * a character count lets multibyte Persian text slip past the limit
     * and get rejected by the API.
     */
    const capped = cutToBytes(text, 60);

    if (currentRow.length) {
      const combined = currentRow[0].text.length + capped.length;
...
<<<<<<< HEAD
  ...
  if (!ascii || ascii.length > 40) {
    throw new Error("Enter a complete order code or mobile number.");
  }

  const persian = ascii.replace(/[0-9]/g, digit =>
...
<<<<<<< HEAD
    ...
    if (access.isOwner && !lastOwner) {
      return [row, [B("🗑 حذف", "deladmin:" + admin.id)]];
    }
