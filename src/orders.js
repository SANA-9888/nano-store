 ...
 *
 * The code alone is not a secret anyone can type from memory: it is printed
 * on the order confirmation, so pairing it with the customer's own phone
 * number is what proves the request belongs to them. Only fields that are
 * already public on the order page are returned — never the internal id,
 * the access hash, or the receipt file ids.
 */
export async function trackOrder(request, env) {
  const body = await request.json().catch(() => ({}));

  const code = String(body?.code || "").trim().toUpperCase();
  const phone = digits(String(body?.phone || "")).trim();

  if (!/^S-[A-Z0-9]{6,20}$/.test(code)) {
    throw new AppError(400, "کد سفارش معتبر نیست.");
  }

  if (!/^09\d{9}$/.test(phone)) {
    throw new AppError(400, "شماره موبایل معتبر نیست.");
  }

  await rateLimit(env, "track:" + phone, 10, 60);

  const order = await one(
    env,
    "SELECT code,name,phone,total,status,payment_status,payment_method,gateway," +
    "created_at,updated_at FROM orders WHERE code=? AND phone=?",
    [code, phone]
  );

  if (!order) {
    throw new AppError(404, "سفارشی با این کد و شماره پیدا نشد.");
  }

  const lines = await rows(
    env,
    "SELECT name,quantity,price FROM order_lines WHERE order_id=" +
    "(SELECT id FROM orders WHERE code=? AND phone=?) ORDER BY id",
    [code, phone]
  );
